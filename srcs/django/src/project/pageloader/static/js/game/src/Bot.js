...
export class Bot {
    ...
    update(dt, balls, closestBall) {
        ...
        for(let i = 0; i < this.mesh.balls.length; i++){
            ...
            if(this.mesh.balls[i].mesh.position.y <= this.mesh.position.y + 0.5 &&
                this.mesh.balls[i].mesh.position.y >= this.mesh.position.y - 0.5 &&
                this.mesh.balls[i].mesh.position.x>= this.mesh.position.x - 5 &&
                this.mesh.balls[i].mesh.position.x<= this.mesh.position.x + 5)
            {   
                // console.log("collisione");
                if(this.mesh.balls[i].mesh.position.x >= this.mesh.position.x - 5 &&
                    this.mesh.balls[i].mesh.position.x <= this.mesh.position.x + 5 &&
                    this.mesh.balls[i].mesh.position.y >= this.mesh.position.y - 0.5 &&
                    this.mesh.balls[i].mesh.position.y <= this.mesh.position.y + 0.5)
                {
                    this.mesh.balls[i].mesh.velocity.y *= -1;
                }
        ...
        if(balls[newClosestBall] != null) { // seguo la palla più vicina
            if(balls[newClosestBall].mesh.position.x > this.mesh.position.x) {
                this.mesh.position.x += dt * this.mesh.speed;
                balls[newClosestBall].mesh.material = RED_MATERIAL;                
            }
            else if(balls[newClosestBall].mesh.position.x < this.mesh.position.x) {
                this.mesh.position.x -= dt * this.mesh.speed;
               balls[newClosestBall].mesh.material = RED_MATERIAL;
            }
